...
void KmerIndex::BuildTranscripts(const ProgramOptions& opt) {
  ...
  std::cerr << "K-mer length: " << k << std::endl;
  ...
  for (auto& fasta : opt.transfasta) {
	...
	std::cerr << "Repeat in FASTA file(s) " << name << " in " << fasta << ", skipping repeat " << std::endl;
      } else {
  ...
  if (polyAcount > 0) {
    std::cerr << "Warning: clipped off poly-A tail (longer than 10)" << std::endl << "        from " << polyAcount << " target sequences" << std::endl;
  }

  
  if (countNonNucl > 0) {
    std::cerr << "Warning: replaced " << countNonNucl << " non-ACGUT characters in the input sequence" << std::endl << "        with pseudorandom nucleotides" << std::endl;
  }
  if (countUNuc > 0) {
    std::cerr << "Warning: replaced " << countUNuc << " U characters with Ts" << std::endl;
  }
...
void KmerIndex::BuildDeBruijnGraph(const ProgramOptions& opt, const std::vector<std::string>& seqs) {
  ...
  std::cerr << "Counting kmers ... "; std::cerr.flush();
  // gather all k-mers
  for (int i = 0; i < seqs.size(); i++) {
  ...
  std::cerr << "Building target de Bruijn graph ... "; std::cerr.flush();
...
void KmerIndex::BuildEquivalenceClasses(const ProgramOptions& opt, const std::vector<std::string>& seqs) {
  ...
  std::cerr << " done" << std::endl;
  std::cerr << "Target de Bruijn graph has " << dbGraph.contigs.size() << " contigs and contains "  << kmap.size() << " kmers " << std::endl;
...
void KmerIndex::write(const std::string& index_out, bool writeKmerTable) {
  ...
  // to write out again.
  std::cerr << "Num_trans: " << num_trans << " Target_names: " << target_names_.size() << std::endl;
...
void KmerIndex::load(ProgramOptions& opt, std::stringstream& logfile_track,bool loadKmerTable) {
  ...
  std::cerr << "Kmer length: " << k << std::endl;
  logfile_track << "Kmer length: " << k << std::endl;
  std::cerr << "Number of targets: " << pretty_num(num_trans)
    << std::endl;
  logfile_track << "Number of targets: " << pretty_num(num_trans)
    << std::endl;
  std::cerr << "Number of kmers: " << pretty_num(kmap_size)
    << std::endl;
  logfile_track << "Number of kmers: " << pretty_num(kmap_size)
    << std::endl;
  ...
  std::cerr << "Number of equivalence classes: "
    << pretty_num(ecmap_size) << std::endl;
  logfile_track << "Number of equivalence classes: "
    << pretty_num(ecmap_size) << std::endl;
      ...
      if (dist >= 2) {
        ...
        if (kit2 != kit_end) {
          ...
          if (found2) {
            ...
            // this is weird, let's try the middle kmer
